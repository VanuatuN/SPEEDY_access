...
C      real prec(ngp), difice(ngp)
      real prec(ngp), snow(ngp), difice(ngp) !added for ACCESOM2 forcing: Natalia 03/09/2026 
...
c     snowfall: precipitation at surface temperature below freezing - added for ACCESOM2 forcing: Natalia 03/09/2026 
      where (ts(:).lt.273.15)
        snow(:) = prec(:)
      elsewhere
        snow(:) = 0.
...
c     snowfall - added for ACCESOM2 forcing: Natalia 03/09/2026
      save2d_2(:,15) = save2d_2(:,15) + snow(:)*86.400
...
c     surface downward radiation - added for ACCESOM2 forcing: Natalia 03/09/2026
      save2d_2(:,13) = save2d_2(:,13) + ssrd(:)
      save2d_2(:,14) = save2d_2(:,14) + slrd(:)
